...
const program: WebGLProgram | null = null

const mvpMatrixLocation: WebGLUniformLocation | null = null
const modelMatrixLocation: WebGLUniformLocation | null = null
const normalMatrixLocation: WebGLUniformLocation | null = null
const uLightPositionLocation: WebGLUniformLocation | null = null
const uViewPositionLocation: WebGLUniformLocation | null = null

const coordinateVAO: WebGLVertexArrayObject | null = null
const triangleVAO: WebGLVertexArrayObject | null = null
const pointsVAO: WebGLVertexArrayObject | null = null
...
const lastCanvasWidth = 0
const lastCanvasHeight = 0
let renderPending = false

const perfText = ref('')
const perfFrames = 0
const perfTotalCost = 0
const perfWindowStart = 0
...
async function renderGl() {
  ...
  const cubeProgram = twgl.createProgramInfo(gl, [
    colorCube.shader.vertex,
    colorCube.shader.fragment,
  ])
  const textureProgram = twgl.createProgramInfo(gl, [
    textureCube.shader.vertex,
    textureCube.shader.fragment,
  ])

  const bufferInfo = twgl.createBufferInfoFromArrays(gl, colorCube.MeshArrays)
  const vao = twgl.createVAOFromBufferInfo(gl, cubeProgram, bufferInfo)
  gl.useProgram(cubeProgram.program)
  gl.bindVertexArray(vao)
  updateProjectionMatrix(gl.canvas.width, gl.canvas.height)
  updateModelMatrix()
  ...
  colorCube.uniforms.u_ModelMatrix = modelMatrix.elements
  colorCube.uniforms.u_MvpMatrix = mvpMatrix.elements
  colorCube.uniforms.u_NormalMatrix = normalMatrix.elements
  twgl.setUniforms(cubeProgram, colorCube.uniforms)
  ...
  const textureBufferInfo = twgl.createBufferInfoFromArrays(gl, {
    Position: colorCube.MeshArrays.Position,
    Normal: colorCube.MeshArrays.Normal,
    TexCoord: textureCube.MeshArrays.TexCoord,
    indices: colorCube.MeshArrays.indices,
  })
  const textureVao = twgl.createVAOFromBufferInfo(gl, textureProgram, textureBufferInfo)
